...
void* SalarySearch(void *arg){
	...
	struct Employee *test = (struct Employee *)arg;
...
void ThreadSpawn(struct Employee *target){
...
struct EmployeeStack* searchMain(void *sourceQueryFromClient){
	...
	struct Employee* pEmployeeStruct = malloc(sizeof (struct Employee)); //Introduced by M Oliver.
